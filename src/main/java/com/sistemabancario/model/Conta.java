...
public class Conta implements Cadastro {
     ...
     * (R01). O número da agência tem um dígito verificador como no CPF, mas isto é
     * outro requisito não definido aqui.
     */
    private String numero;

    /**
     * Contas devem ser instanciadas como "Conta Corrente" e não como "Poupança".
     * (R02) O valor padrão para atributos boolean é false, assim não precisamos
     * escrever código adicional para definir "poupanca" como false. Mas é preciso
     * escrever o teste para verificar tal situação. Com isto, buscamos detectar se
     * uma alteração no código fizer com que este requisito deixe de ser atendido.
     */
     ...
     * disponível. Somente contas especiais podem ter limite, ou seja, o limite de
     * contas "não especiais" não pode ser maior que zero (R03).
     */
     ...
     * NullPointerException. Um teste deve verificar se, após instanciar uma conta
    ...
    /**
     * Adiciona uma nova movimentação à lista de {@link #movimentacoes}. (R05) Se a
     * movimentação estiver confirmada, seu valor deve ser:
     * <ul>
     ...
     * caso o valor seja menor ou igual ao saldo total. Após realizar um saque, o
     * saldo deve ser atualizado.
     *
     * <p>
     * Se for tentando realizar dois saques ao mesmo tempo com valor igual ao saldo
     * total, somente o primeiro saque deve ser permitido. Isto evita que um cliente
     * que possua dois cartões da mesma conta tente retirar mais dinheiro do que há
     * disponível na conta. Ele poderia tentar fazer isso solicitando a ajuda de
     * outra pessoa. Assim, as duas pessoas poderiam tentar ir em caixas diferentes
     * ao mesmo tempo para tentar realizar um saque em duplicidade.
     * </p>
     *
    ...
    /**
     * Adiciona uma nova movimentação de depósito em dinheiro tanto realizada por um
     * funcionário quanto em um caixa eletrônico, que deve ser confirmada
     * automaticamente. Considera-se que todos os caixas eletrônicos do banco têm
    ...
    /**
     * Adiciona uma nova movimentação de depósito em cheque (que deve ser confirmada
     * posteriormente por um funcionário do banco).
     *
     * @param valor valor a ser depositado (deve ser um valor positivo)
     */
    ...
    public void setLimite(double limite) {
        ...
        if (!especial && limite > 0) {
            throw new IllegalStateException("Somente contas especiais podem alterar limite");
        }
